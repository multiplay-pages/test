const state = {
  ...
  retentionMonths: 1, // Nowa zmienna stanu dla Promocji Utrzymaniowej
...
const compatibilityMatrix = {
  ...
  choose_gift: ["banner"],
  retention: ["banner"] // Możliwość łączenia Promocji Utrzymaniowej z Banerową
};
...
function normalizeState() {
  ...
  // Zabezpieczenie na input z miesiącami dla Promocji Utrzymaniowej (wartość od 1 do 24)
...
function getPromotionDuration(type) {
  ...
  if (type === "retention") return Math.min(state.retentionMonths, state.commitment); // Czas trwania P. Utrzymaniowej
...
function applyPromotionsForMonth(baseCalc, month, activePromotions, subscriptionSchedule) {
  ...
  // Obniżenie rachunku do 1 zł w przypadku darmowych/promocyjnych miesięcy (w tym P. Utrzymaniowej)
...
function renderPromotionConfigurator() {
  ...
  // Dodana "retention" do listy renderowanych opcji
  ...
  // Renderowanie pola input dla Promocji Utrzymaniowej
...
function bindPromotionControls() {
  ...
  document.addEventListener("change", (event) => {
    ...
    // Nowy nasłuchiwacz na nowo dodane pole
